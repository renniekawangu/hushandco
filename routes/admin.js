...
router.get('/dashboard', isAdmin, async (req, res) => {
    try {
        ...
        const messagesList = await Message.find({}).sort({ createdAt: -1 }).limit(20);
...
router.post('/messages/mark-read/:id', isAdmin, async (req, res) => {
    try {
...
router.get('/messages', isAdmin, async (req, res) => {
    try {
        ...
        const messagesList = await Message.find(filter).sort({ createdAt: -1 }).limit(100);
...
router.get('/messages/json', isAdmin, async (req, res) => {
    try {
