...
router.post('/place-order', isAuthenticated, async (req, res) => {
    try {
        ...
        const orderItems = await Promise.all(req.session.cart.map(async (item) => {
...
router.post('/process-payment', isAuthenticated, async (req, res) => {
    try {
        ...
        if (!paymentMethod) {
            return res.status(400).render('error', { error: 'Payment method is required' });
        }

        // Get the latest order for this user
        const order = await Order.findOne({ user: req.session.userId }).sort({ createdAt: -1 });
        ...
        if (paymentMethod === 'AirtelMoney') {
            ...
            res.status(501).render('error', { error: 'Credit card payments coming soon' });
        } else if (paymentMethod === 'BankTransfer') {
            res.status(501).render('error', { error: 'Bank transfer payments coming soon' });
        } else {
