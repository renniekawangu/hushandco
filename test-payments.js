...
async function runTests() {
    ...
    test('AIRTEL_CLIENT_SECRET is set', () => {
        assert(process.env.AIRTEL_CLIENT_SECRET, 'AIRTEL_CLIENT_SECRET not found in .env');
    });
    ...
    test('AIRTEL_MERCHANT_ID is set', () => {
        assert(process.env.AIRTEL_MERCHANT_ID, 'AIRTEL_MERCHANT_ID not found in .env');
    });
    ...
    test('Service has required methods', () => {
        assert(typeof service.initiatePayment === 'function', 'Missing initiatePayment method');
        assert(typeof service.getPaymentStatus === 'function', 'Missing getPaymentStatus method');
        assert(typeof service.formatPhoneNumber === 'function', 'Missing formatPhoneNumber method');
        assert(typeof service.verifyWebhookSignature === 'function', 'Missing verifyWebhookSignature method');
    });
    ...
    test('Generate unique merchant transaction IDs', () => {
        ...
        assert(id1.includes(process.env.AIRTEL_MERCHANT_ID), 'ID does not contain merchant ID');
    });

    // Test 5: Configuration validation
    log('\nTesting Configuration...', 'yellow');
    test('Service configuration is correct', () => {
        assert(service.clientId === process.env.AIRTEL_CLIENT_ID, 'Client ID not configured');
        assert(service.merchantId === process.env.AIRTEL_MERCHANT_ID, 'Merchant ID not configured');
        assert(service.environment === 'sandbox' || service.environment === 'prod', 'Invalid environment');
    });
    ...
    test('Webhook verification method exists', () => {
        ...
        assert(typeof result === 'boolean', 'Webhook verification should return boolean');
    });
